...
func main() {
	...
	root := "0x98386b9f0ebc91c937e28cb969d867de8377a41770cd1d7baead7ae2fe0ecde4"
	rootBytes := common.HexToHash(root).Bytes()
	rootValue, err := originDB.Get(rootBytes, nil)
	if err != nil {
		panic(err)
	}
	//log.Println("value: ", value)

	//rootID := trie.TrieID(common.HexToHash(root))
	rootNode, err := decodeNodeUnsafe(rootBytes, rootValue)
	if err != nil {
		panic(err)
	}
	travelTrie(originDB, targetDB, rootNode, rootValue)
...
func travelTrie(originDB, targetDB *leveldb.DB, node node, nodeData []byte) {
	switch n := node.(type) {
	case nil:
		return
	case *shortNode:
		travelTrie(originDB, targetDB, n.Val, nil)
		hash := common.BytesToHash(n.Key)
		value, err := originDB.Get(n.Key, nil)
		log.Println("1", hash, value, err)
	case *fullNode:
		for _, v := range n.Children {
			travelTrie(originDB, targetDB, v, nil)
		}
	case valueNode:
		log.Println(3, n[:])
		handleKey(originDB, targetDB, n[:], nil)
	case hashNode:
		handleKey(originDB, targetDB, n, nil)
	default:
		log.Println(5, reflect.TypeOf(node))
	}
}
...
func handleKey(originDB, targetDB *leveldb.DB, key []byte, value []byte) {
	...
	case len(key) == common.HashLength:
		...
		node, err := decodeNodeUnsafe(value, v)
		//log.Println("hash, ", len(v), node == nil)
		travelTrie(originDB, targetDB, node, v)
